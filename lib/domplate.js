...
DomplateTag.prototype =
{
    ...
    compileMarkup: function()
    {
        ...
        var fnBlock = ['(function (__code__, __context__, __in__, __out__, __index__'];
        ...
        function __link__(tag, code, outputs, args, index)
        {
            if (!tag || !tag.tag)
                return;
            ...
            var tagOutputs = [];
            var markupArgs = [code, tag.tag.context, args, tagOutputs, index];
        ...
        function __loop__(iter, outputs, fn)
        {
            ...
            try
            {
                ...
                while (1)
                {
                    ...
                    fn.apply(this, [value, itemOuts, i]);
                    i++;
                }
            }
            catch (exc)
...
//system.print(js,'JS');
    ...
    generateMarkup: function(topBlock, topOuts, blocks, info)
    {
        ...
        topBlock.push(', (__index__)?" _did=\\""+__index__+"\\"":""');
...
DomplateLoop.prototype = copyObject(DomplateTag.prototype,
{
    ...
    generateMarkup: function(topBlock, topOuts, blocks, info)
    {
        ...
        blocks.push('    __loop__.apply(this, [', iterName, ', __out__, function(', this.varName, ', __out__, __index__) {');
        this.generateChildMarkup(topBlock, topOuts, blocks, info);
        this.addCode(topBlock, topOuts, blocks);
        blocks.push('    }]);');
...
print(html);
    ...
    generateStaticEmbed: function(args, options, self)
    {
        this.tag.compile();
...
print("this.tag.renderMarkup");
print(this.tag.renderMarkup);
print("this.tag.renderDOM");
print(this.tag.renderDOM);
//print("outputs");
//dump(outputs);
        
        var embed = {"html": [], "outputs": outputs, "eventMap": {}};

        // process outputs to swap event functions for pointers that we
        // can send to the server to trigger same functions in same context
        // on server and return the result
        
        this.generateStaticEmbed_traverseOutputs(outputs, embed.eventMap);
...
//html += "<script>alert('hello World')</script>";
        ...
        embed.html.push("function(womb) {");
                
        embed.html.push("var options = " + JSON.encode(options).replace(/'/g, "\\'") + ";");
        
        // render the rep
        embed.html.push("womb.innerHTML = '" + html.replace(/'/g, "\\'") + "';");

        var __path__ = function(parent, offset)
        {
            var root = parent;
            for (var i = 2; i < arguments.length; ++i)
            {
        ...
        embed.html.push("var __path__ = " + UTIL.trim(__path__) + ";");

        var __bind__ = function(object, fn)
        {
//            return function(event) { return fn.apply(object, [event]); }
            return function(event) {
//                console.log(event);

// traverse up to womb to determine target and path
var element = event.target;
var path  = [];
while(element!=womb) {
    for( var i=0 ; i<element.parentNode.childNodes.length ; i++ ) {
        if(element.parentNode.childNodes[i]===element && element.hasAttribute("_did")) {
            path.push(element.getAttribute("_did"));
        }
    }
...
xhReq.open("GET", options.eventBaseURL + "?nodeId="+path.join(",")+"&eventId="+fn, false);
xhReq.send(null);
                
                console.log(""+xhReq.responseText);            
            }
        }
        embed.html.push("var __bind__ = " + UTIL.trim(__bind__) + ";");

        var __loop__ = function (iter, fn)
        {
            var nodeCount = 0;
            for (var i = 0; i < iter.length; ++i)
            {
        ...
        embed.html.push("var __loop__ = " + UTIL.trim(__loop__) + ";");


        var __link__ = function (node, tag, args)
        {
            if (!tag || !tag.tag)
                return;

            console.log("compile & render TAG");
//            tag.tag.compile();
...
//            return oo;
        ...
        embed.html.push("var __link__ = " + UTIL.trim(__link__) + ";");


        // attach events to be proxied to the server if applicable
        embed.html.push("var renderDOM = " + UTIL.trim(this.tag.renderDOM) + ";");

        embed.html.push("var root = womb.firstChild;");

//        var domArgs = [root, this.tag.context, 0];
        embed.html.push("var domArgs = [root, null, 0];");

//        domArgs.push.apply(domArgs, this.tag.domArgs);
        embed.html.push("domArgs.push.apply(domArgs, []);");

//        domArgs.push.apply(domArgs, outputs);
        embed.html.push("domArgs.push.apply(domArgs, " + JSON.encode(embed.outputs).replace(/'/g, "\\'") + ");");


        embed.html.push("console.log(domArgs);");
        
//print("this.tag.subject: "+this.tag.subject);     
//dump(this.tag.subject);   
//        this.tag.renderDOM.apply(self ? self : this.tag.subject, domArgs);
        embed.html.push("renderDOM.apply(root, domArgs);");

        embed.html.push("}");
        
        embed.html = embed.html.join("\n");
        
        return embed;
    },

    generateStaticEmbed_traverseOutputs: function(outputs, map, path) {
        path = path || [];
        for(var i=0 ; i<outputs.length ; i++ ) {
            
            if(UTIL.isArrayLike(outputs[i])) {
                this.generateStaticEmbed_traverseOutputs(outputs[i], map, path.concat(i));
            } else
            if(typeof outputs[i] == "function") {
//print("("+path.concat(i)+") FUNCTION:" + item);       
                var key = path.concat(i).join(",");
